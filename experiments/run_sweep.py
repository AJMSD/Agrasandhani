...
def parse_args() -> SweepConfig:
    ...
    parser.add_argument("--variants", default="v0,v2,v4")
    parser.add_argument("--qos", default="0,1")
    parser.add_argument("--scenarios", default="clean,bandwidth_200kbps,loss_2pct,delay_50ms_jitter20ms,outage_5s")
    parser.add_argument("--data-file", type=Path, default=BASE_DIR / "simulator" / "datasets" / "intel_lab_sample.csv")
    ...
    parser.add_argument("--burst-enabled", action="store_true", default=True)
    parser.add_argument("--burst-start-s", type=int, default=5)
    parser.add_argument("--burst-duration-s", type=int, default=10)
    ...
    return SweepConfig(
        ...
        qos_values=[int(item.strip()) for item in args.qos.split(",") if item.strip()],
